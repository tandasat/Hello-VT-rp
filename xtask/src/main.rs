...
fn build(release: bool) -> Result<(), DynError> {
    ...
    cargo_run(Action::Build, Package::UefiClient, profile)?;
    cargo_run(Action::Build, Package::OsClient, profile)
}

fn clippy() -> Result<(), DynError> {
    cargo_run(Action::Clippy, Package::Hypervisor, Profile::Debug)?;
    cargo_run(Action::Clippy, Package::UefiClient, Profile::Debug)?;
    cargo_run(Action::Clippy, Package::OsClient, Profile::Debug)?;
